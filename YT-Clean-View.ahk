...
; 3. Switchs back to original tab
...
ogTab:= cUIA.GetTab().Name ; 'Name' is required for SelectTab() below

; Open link in DETURL webpage of OS's default browser
cleanLink:= RegExReplace(Link[1], CLEANneedle)
Run(DETURL cleanLink)
cUIA.WaitPageLoad(tURL,,500,1,True) ; SetTitleMatchMode = begins with >> browser may sometimes append additional info after title?
